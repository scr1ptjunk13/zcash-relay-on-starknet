...
const EQUIHASH_SOLUTION_SIZE_BYTES: u32 = 1344;

// Helper function for 2^n with u32 result
fn pow32(n: u32) -> u32 {
    if n == 0 { return 1; }
    if n == 1 { return 2; }
    if n == 2 { return 4; }
...
/// 
/// # Arguments
/// * `header` - Block header bytes (140 bytes + 32 byte nonce = 172 bytes)
/// * `n`, `k` - Equihash parameters (200, 9 for Zcash)
/// * `indices` - The indices to process (e.g., 64 indices for a batch)
/// 
/// # Returns
/// * Array of (hash_input_index, full_blake2b_output) pairs
/// * Only computes each unique hash_input_index once
pub fn compute_unique_hashes(
    header: Array<u8>,
    n: u32,
    k: u32,
    indices: Span<u32>
...
/// Create a leaf node from a cached Blake2b hash (no Blake2b computation)
/// 
/// # Arguments
/// * `cached_hashes` - Pre-computed hashes from compute_unique_hashes()
/// * `n`, `k` - Equihash parameters
/// * `idx` - The solution index
pub fn make_leaf_from_cached(
    cached_hashes: @Array<(u32, Array<u8>)>,
    n: u32,
    k: u32,
    idx: u32
    ...
    // Determine which cached hash to use and which subindex
    let hash_input_index: u32 = idx / indices_per;
    let subindex: u32 = idx % indices_per;
    
    // Find the cached hash
    let full_hash = find_cached_hash(cached_hashes, hash_input_index);
...
/// Create multiple leaf nodes efficiently using hash caching.
/// This is the main optimized function - computes Blake2b only for unique hash_input_indices.
/// 
/// For 64 indices: only 32 Blake2b calls instead of 64 (50% reduction)
/// 
/// # Arguments
/// * `header` - Full header bytes (with nonce)
/// * `n`, `k` - Equihash parameters (200, 9 for Zcash)
/// * `indices` - Solution indices to process
/// 
/// # Returns
/// * Array of EquihashNode leaves
pub fn make_leaves_batch_optimized(
    header: Array<u8>,
    n: u32,
    k: u32,
    indices: Span<u32>
) -> Array<EquihashNode> {
    // Step 1: Compute unique hashes (the expensive part, now deduplicated)
    let cached_hashes = compute_unique_hashes(header, n, k, indices);
    
    // Step 2: Create leaves using cached hashes (cheap)
    let mut leaves: Array<EquihashNode> = array![];
    let mut i: usize = 0;
    
    while i < indices.len() {
        let idx = *indices[i];
        let leaf = make_leaf_from_cached(@cached_hashes, n, k, idx);
        leaves.append(leaf);
        i += 1;
    };
...
pub fn collision_bit_length(n: u32, k: u32) -> u32 {
    n / (k + 1_u32)
}

pub fn collision_byte_length(n: u32, k: u32) -> usize {
...
pub fn has_collision(ref a: EquihashNode, ref b: EquihashNode, len: usize) -> bool {
...
pub fn indices_before(a: @EquihashNode, b: @EquihashNode) -> bool {
...
pub fn distinct_indices(a: @EquihashNode, b: @EquihashNode) -> bool {
...
pub fn from_children(mut a: EquihashNode, mut b: EquihashNode, trim: usize) -> EquihashNode {
...
pub fn is_zero_prefix(node: EquihashNode, len: usize) -> bool {
...
pub fn make_leaf(n: u32, k: u32, header_span: Array<u8>, idx: u32) -> EquihashNode {
...
// Returns (ok, indices).
pub fn indices_from_minimal_bytes(n: u32, k: u32, minimal: Array<u8>) -> (bool, Array<u32>) {
...
// NOTE: This function is commented out because it depends on a Header type
// that doesn't exist in our current codebase. We use ZcashBlockHeader instead.
//
...
// NOTE: Commented out because it depends on Header type
...
mod tests {
    use core::array::ArrayTrait;
    use crate::utils::bit_shifts::{shl_u64 as shl64, shr_u64 as shr64, shl_u128};
    ...
    fn minimal_from_indices(n: u32, k: u32, indices: Array<u32>) -> Array<u8> {
        ...
        while i < len {
            ...
            let idx_u128: u128 = idx_u32.into();
            let mask: u128 = shl_u128(1_u128, bits_per_index) - 1_u128;
            let val: u128 = idx_u128 & mask;

            // Append bits_per_index bits of val (MSB-first) to the accumulator.
            acc_value = shl_u128(acc_value, bits_per_index) | val;
            acc_bits = acc_bits + bits_per_index;

            // Flush bytes while we have >= 8 bits.
            while acc_bits >= 8_u32 {
                let shift: u32 = acc_bits - 8_u32;
                let byte_u128: u128 = (acc_value / shl_u128(1_u128, shift)) & 0xff_u128;
                let byte: u8 = byte_u128.try_into().unwrap();
                out.append(byte);

                // Remove those top 8 bits.
                let mask_acc: u128 = if shift == 0_u32 {
                    0_u128
                } else {
                    shl_u128(1_u128, shift) - 1_u128
                };
        ...
        if acc_bits > 0_u32 {
            ...
            let byte_u128: u128 = shl_u128(acc_value, shift) & 0xff_u128;
            let byte: u8 = byte_u128.try_into().unwrap();
            out.append(byte);
        }
