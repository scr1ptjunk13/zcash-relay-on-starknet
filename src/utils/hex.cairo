...
// Gets `Digest` from reversed `ByteArray` (for Bitcoin/Zcash display format).
...
// Gets `Digest` from `ByteArray` (for internal computation format - no reversal).
pub fn hex_to_hash(hex_string: ByteArray) -> Digest {
    let mut result: Array<u32> = array![];
    let mut i = 0;
    let mut unit: u32 = 0;
    let len = hex_string.len();
...
/// This is ZCASH-SPECIFIC for handling Equihash solutions (1344 bytes)
pub fn hex_to_bytes_array(hex_string: ByteArray) -> Array<u8> {
    let num_characters = hex_string.len();
    assert(num_characters % 2 == 0, 'Invalid hex string length');
...
#[cfg(test)]
mod tests {
    use super::super::hash::Digest;
    use super::{from_hex, hex_to_hash_rev, hex_to_hash, to_hex};
    ...
    fn test_hex_to_hash() {
        ...
        let hex = "123456789abcdef01122334455667788aabbccddeeff00112233445566778899";
        assert_eq!(hash, hex_to_hash(hex));
    }
}