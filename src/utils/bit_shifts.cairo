...
use core::num::traits::{One, Zero, WrappingMul};
...
/// Performs a bitwise left shift on a `u64` value by a specified number of bits.
/// This specialized version offers optimal performance for `u64` types.
///
/// # Arguments
/// * `self` - The `u64` value to be shifted
...
/// 
/// # Note
/// * Uses wrapping multiplication to handle overflow (wraps modulo 2^64)
#[inline(always)]
pub fn shl_u64(self: u64, shift: u32) -> u64 {
    self.wrapping_mul(pow2(shift))
}

/// Performs a bitwise left shift on a `u128` value by a specified number of bits.
///
/// # Arguments
...
///
/// # Note
/// * Uses wrapping multiplication to handle overflow (wraps modulo 2^128)
#[inline(always)]
pub fn shl_u128(self: u128, shift: u32) -> u128 {
    if shift == 0 {
        return self;
    }
    if shift >= 128 {
        return 0;  // Would overflow completely
    }
    // Use fast_pow for u128 exponentiation
    let pow_val: u128 = fast_pow(2_u128, shift);
    self.wrapping_mul(pow_val)
}
