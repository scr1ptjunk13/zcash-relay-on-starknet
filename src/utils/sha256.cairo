...
mod tests {
    ...
    #[test]
    fn test_cairo_sha256_basic() {
        // Basic test - just verify the function runs without error
        let mut input: ByteArray = "";
        let _result = compute_sha256_byte_array(@input);
        // If we got here, the function works
        assert(true, 'sha256 basic test passed');
    }
}
