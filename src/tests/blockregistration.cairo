...
fn verify_block_incremental(
    ...
    // Step 2: Verify leaves in 8 batches (64 leaves each)
    println!("Step 2: Verify leaves in 8 batches");
...
fn make_block0() -> ZcashBlockHeader {
...
// ============================================================================
// GAS PROFILING TESTS (single logical step each)
// ============================================================================

#[test]
fn test_gas_start_only() {
    let utu = deploy_relay();
    let timestamp0: u32 = 1477641360;
    start_cheat_block_timestamp(utu.contract_address, timestamp0.into());

    let block0: ZcashBlockHeader = make_block0();

    // STEP 1: Start verification only
    let _verification_id = utu.start_block_verification(block0);
}

#[test]
fn test_gas_single_leaf_batch_0() {
    let utu = deploy_relay();
    let timestamp0: u32 = 1477641360;
    start_cheat_block_timestamp(utu.contract_address, timestamp0.into());
    ...
    // STEP 1: Start verification
    let verification_id = utu.start_block_verification(block0);

    // STEP 2: First leaf batch only (batch_id = 0)
    utu.verify_leaves_batch(verification_id, 0, block0);
}

#[test]
fn test_gas_tree_only() {
    let utu = deploy_relay();
    let timestamp0: u32 = 1477641360;
    start_cheat_block_timestamp(utu.contract_address, timestamp0.into());
    ...
    // STEP 2: Verify all leaf batches (4 batches of 128 leaves each)
    let mut batch_id: u32 = 0;
    while batch_id < 4 {
        utu.verify_leaves_batch(verification_id, batch_id, block0);
        batch_id += 1;
    };

    // STEP 3: Tree build only
    utu.verify_tree_all_levels(verification_id, block0);
}

#[test]
fn test_gas_finalize_only() {
    let utu = deploy_relay();
    let timestamp0: u32 = 1477641360;
    start_cheat_block_timestamp(utu.contract_address, timestamp0.into());
    ...
    // STEP 4: Finalize only
    let _result = utu.finalize_block_verification(verification_id, block0);
}
